...
function ExtractText(colors: tRGBArray; w,h: integer): TNormArray;
const
  GradientMax = 5.0;
...
begin
    ...
    begin
        Writeln('Usage: ocr [-t] maskpath sourceimage');
        exit;
    ...
    begin
        ...
        begin
            writeln('Unknown parameter: ' + paramstr(1));
            exit;
        end;
    end;
    ocrdata:= InitOCR(paramstr(i));
    bitmap:= readbmp(paramstr(i+1));
    if t then
    begin
        writeln('Running time tests...');
        tick:= now;
        for i:= 1 to 500 do
        begin
            ocrDetect(ExtractText(bitmap.data,bitmap.width,bitmap.height),bitmap.width,bitmap.height,ocrdata);
        end;
        time:= trunc((now - tick) * 24 * 60 * 60 * 1000);
        write('OCR took: ');
        writeln(time / 500);
    end;
    writeln(ocrDetect(ExtractText(bitmap.data,bitmap.width,bitmap.height),bitmap.width,bitmap.height,ocrdata));
end.
