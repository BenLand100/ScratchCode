...
numbs = {'1':0,'2':1,'3':2,'4':3,'5':4,'6':5,'7':6,'8':7,'9':8,'T':9,'J':10,'Q':11,'K':12,'A':13}
...
rawcards =  hands.replace('\n',' ').split(' ')
rawhands = [rawcards[i:i+5] for i in range(0,len(rawcards),5)]
cards = [(numbs[c[0]],suits[c[1]]) for c in rawcards]
hands = [cards[i:i+5] for i in range(0,len(cards),5)]

ranks = []
for h in hands:
    samesuit = len(set([c[1] for c in h])) == 1
    nlist = [c[0] for c in h]
    groups = sorted(filter(lambda x: x[1] > 1, [(n, nlist.count(n)) for n in set(nlist)]),key=lambda x: x[1])
    highs = sorted(nlist)
    straight = len(set([highs[i]-i for i in range(5)])) == 1
    rank = 0
    if samesuit and straight and highs[-1] == 11: #royal flush
        rank = 9
    elif samesuit and straight: #straight flush
        rank = 8
    elif len(groups) == 1 and groups[-1][1] == 4: #four of a kind
        rank = 7
    elif len(groups) == 2 and groups[-1][1] == 3 and groups[-2][1] == 2: #full house
        rank = 6
    elif samesuit: #flush
        rank = 5
    elif straight: #straight
        rank = 4
    elif len(groups) == 1 and groups[-1][1] == 3: #three of a kind
        rank = 3
    elif len(groups) == 2 and groups[-1][1] == groups[-2][1] == 2: #two pair
        rank = 2
    elif len(groups) == 1 and groups[-1][1] == 2: #pair
        rank = 1
    ranks.append((rank,samesuit,straight,groups,highs))

games = [(ranks[i],ranks[i+1]) for i in range(0,len(ranks),2)]

p1wins = 0
for j,h in enumerate(games):
    p1,p2 = h
    #print rawhands[j*2],rawhands[j*2+1]
    if p1[0] == p2[0]: #same hand
        typ = p1[0]
        if typ == 0:
            i = -1
            while p1[4][i] == p2[4][i]: i-=1
            if p1[4][i] > p2[4][i]:
                p1wins += 1
        elif typ == 1:
            if p1[3][-1][0] == p2[3][-1][0]:
                i = -1
                while p1[4][i] == p2[4][i]: i-=1
                if p1[4][i] > p2[4][i]:
                    p1wins += 1
            elif p1[3][-1][0] > p2[3][-1][0]:
                p1wins += 1
        elif typ == 2:
            if p1[3][-1][0] == p2[3][-1][0]:
                if p1[3][-2][0] == p2[3][-2][0]:
                    i = -1
                    while p1[4][i] == p2[4][i]: i-=1
                    if p1[4][i] > p2[4][i]:
                        p1wins += 1
                elif p1[3][-2][0] > p2[3][-2][0]:
                    p1wins += 1
            elif p1[3][-1][0] > p2[3][-1][0]:
                p1wins += 1
        elif typ == 3:
